...
import { CHARACTER_COLON_REGEX, SCENE_REGEX, TRANSITION_REGEX, PARENTHETICAL_REGEX, OS_DIALOGUE_REGEX, COLOR_TAG_REGEX, SUMMARY_REGEX, NOTE_REGEX } from './main';
...
export class DocxExporter {
    static async exportToDocx(text: string, title: string): Promise<Buffer> {
        ...
        for (let lineText of lines) {
            ...
            // 3. Character / OS / VO
            const isCharacter = trimmed.startsWith('@') ||
                (/^[A-Z0-9\s-]{1,30}(\s+\([^)]+\))?$/.test(trimmed) && trimmed.length > 0) ||
                (/^[\u4e00-\u9fa5A-Z0-9\s-]{1,30}$/.test(trimmed)) ||
                CHARACTER_COLON_REGEX.test(trimmed);

            if (isCharacter) {
                let charName = trimmed.startsWith('@') ? trimmed.substring(1).trim() : trimmed;
